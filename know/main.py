...
"""
# content2="""IN light of the recent consultative meeting organised by Pildat and UNDP, it is high time Pakistan’s political landscape embraced a much-needed reform: limiting the number of seats an election candidate can contest.
...
data_frame=pd.read_csv("/home/pcn/Desktop/NAaaS/Scrapper/2024/2024-01-03/business.csv")
# content3="""In 2022, John Smith, the CEO of XYZ Corporation, attended the United Nations General Assembly in New York City, where he discussed climate change with Angela Merkel, the Chancellor of Germany.He highlighted the company's partnership with SpaceX to Angela Merkel, headed by Elon Musk, to develop innovative sustainable energy solutions for the future."""
json_data=data_frame['Detail']
concatenated_text = ' '.join(json_data)
...
# #     content=file.read()
Miner.main(concatenated_text)
with open('graph_data.json', 'r') as file:
    data = json.load(file)
data_nodes = data['edges']
keywords = [i['label'] for i in data_nodes if i['label'] not in ['frequency', 'Location', 'Person', 'related', 'keyword']]
print(keywords)
json_filename = 'keywords.json'
print(len(data_frame))
with open(json_filename, 'w') as json_file:
    json.dump(keywords, json_file)
format_json_file("keywords.json")
